...
class GetACategoryOfRecyclePoints @Inject constructor(
    private val repository: TimisoaraRecyclePointsRepository
) {
    operator fun invoke(categoryName: String): Flow<Resource<List<RecyclePoint>>> = flow {
        try {
            emit(Resource.Loading<List<RecyclePoint>>())

            val result = repository.getPointsOfACategory(categoryName).result.toListOfRecyclePoint()
